...
rt_hw_context_switch_to    PROC																;**(6)
    ...
    LDR     r1, =rt_interrupt_to_thread             ;将rt_interrupt_to_thread的地址加载到r1 **(8-1)
    STR     r0, [r1]                                ;将r0的值存储到rt_interrupt_to_thread	**(8-2)

    ; 设置rt_interrupt_from_thread的值为0，表示启动第一次线程切换							 ;**(9)
    LDR     r1, =rt_interrupt_from_thread           ;将rt_interrupt_from_thread的地址加载到r1 **(9-1)
    MOV     r0, #0x0                                ;配置r0等于0							  **(9-2)
    STR     r0, [r1]                                ;将r0的值存储到rt_interrupt_from_thread	  **(9-3)

    ; 设置中断标志位rt_thread_switch_interrupt_flag的值为1									;**(10)
    LDR     r1, =rt_thread_switch_interrupt_flag    ;将rt_thread_switch_interrupt_flag的地址加载到r1 **(10-1)
    MOV     r0, #1                                  ;配置r0等于1									**(10-2)
    STR     r0, [r1]                                ;将r0的值存储到rt_thread_switch_interrupt_flag	**(10-3)
